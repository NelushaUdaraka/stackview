...
function formatTemplate(body: string): string {
  try {
    ...
    // YAML: basic indentation normalisation — add blank lines before top-level keys
    return body
      .split('\n')
...
export default function CreateStackModal({ onClose, onCreated }: Props) {
  ...
  const validateTemplate = async (body: string) => {
    ...
    if (res.success && res.data) {
      ...
      const incoming: Param[] = (res.data.parameters ?? []).map((p: any) => ({
        ...
        value: p.defaultValue ?? '',          // pre-fill with default
      }))
      setParams(prev => {
        const map = new Map(prev.map(x => [x.key, x]))
        return incoming.map(ip => map.has(ip.key) ? { ...ip, value: map.get(ip.key)!.value } : ip)
      })
      return true
    } else {
  ...
  const updateParam = (id: string, val: string) =>
    setParams(p => p.map(x => x.id === id ? { ...x, value: val } : x))
  ...
  const handleCreate = async () => {
    ...
    const ps = params.filter(p => p.key.trim()).map(p => ({ key: p.key, value: p.value }))
    const res = await window.electronAPI.cfnCreateStack(stackName.trim(), templateBody, ps, capabilities)
    setLoading(false)
    if (res.success) { onCreated() } else { setSubmitError(res.error ?? 'Failed to create stack') }
  }

  const canGoNext = (): boolean => {
    if (step === 0) return templateBody.trim().length > 0
    if (step === 2) return stackName.trim().length >= 1
    return true
  }
      ...
      <div className="w-full max-w-2xl rounded-2xl shadow-2xl border border-theme flex flex-col max-h-[90vh]" style={{ backgroundColor: 'rgb(var(--bg-base))' }}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-theme shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-bold text-1">Create Stack</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === 0 && (
            <div className="space-y-4">
              <p className="text-xs text-3 leading-relaxed">Provide a CloudFormation template in YAML or JSON format.</p>
              <div className="flex flex-wrap gap-2">
                <button onClick={() => { setTemplateSource('paste'); setFileName('') }}
                  ...
                  <ClipboardPaste size={13} /> Paste template
                </button>
            ...
            <div className="space-y-3">
              {params.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center text-3">
                  ...
                  <p className="text-sm font-semibold text-2">No parameters required</p>
                  <p className="text-xs mt-1">This template has no configurable parameters.</p>
                </div>
              ) : (
                <>
                  <p className="text-xs text-3">Fill in the parameter values. Defaults are pre-filled; allowed values are shown as a dropdown.</p>
                  {params.map(p => (
                    <div key={p.id} className="space-y-1.5 p-3 rounded-xl border border-theme bg-raised/20">
                      <div className="flex items-center gap-2">
                        <label className="text-xs font-semibold text-1">{p.key}</label>
                        {p.defaultValue && (
                          <span className="text-[10px] text-3 font-mono bg-raised px-1.5 py-0.5 rounded">
                      ...
                      {p.allowedValues.length > 0 ? (
                        <select value={p.value} onChange={e => updateParam(p.id, e.target.value)}
                          className="input-base text-xs py-1.5 w-full">
                          {!p.defaultValue && <option value="">— select a value —</option>}
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type="text" value={stackName} onChange={e => setStackName(e.target.value)}
                  placeholder="my-production-stack" className="input-base w-full" autoFocus />
                <p className="text-[10px] text-3 mt-1">Must be unique in this region. Use letters, numbers, and hyphens.</p>
              </div>
              ...
              <div>
                  ...
                  {CAPABILITIES.map(c => (
                    <label key={c} className="flex items-start gap-2.5 cursor-pointer p-2.5 rounded-lg hover:bg-raised transition-colors">
                      <input type="checkbox" checked={capabilities.includes(c) || detectedCaps.includes(c)}
                        ...
                        className="mt-0.5 rounded border-theme bg-base text-orange-500 focus:ring-orange-500" />
                      <div>
                ...
                {detectedCaps.length > 0 && (
                  <p className="text-[10px] text-orange-500 mt-2">⚠ Template requires: {detectedCaps.join(', ')}</p>
                )}
              </div>
          ...
          {step === 3 && (
            <div className="space-y-4">
              <p className="text-xs text-3">Review your configuration before creating the stack.</p>
              <ReviewSection title="Stack"><ReviewRow label="Name" value={stackName || '—'} /></ReviewSection>
              <ReviewSection title="Template">
                <ReviewRow label="Source" value={fileName || 'Pasted content'} />
              ...
              {params.length > 0 && (
                <ReviewSection title="Parameters">
                  {params.map(p => <ReviewRow key={p.id} label={p.key} value={p.value || `(default: ${p.defaultValue || 'empty'})`} />)}
                </ReviewSection>
              )}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-theme shrink-0">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <button onClick={handleCreate} disabled={loading}
              ...
              {loading && <Loader2 size={12} className="animate-spin" />} Create Stack
            </button>
          )}
        </div>
      </div>
