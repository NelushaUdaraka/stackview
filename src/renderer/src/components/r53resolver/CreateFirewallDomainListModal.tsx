...
export default function CreateFirewallDomainListModal({ onCreated, onClose }: Props) {
  ...
  const handleCreate = async () => {
    if (!isValid) return
    setLoading(true)
    const res = await window.electronAPI.r53rCreateFwDomainList(name.trim())
    if (res.success) { onCreated() }
    else showToast('error', res.error ?? 'Failed to create domain list')
    setLoading(false)
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-2xl border border-theme shadow-2xl flex flex-col" style={{ backgroundColor: 'rgb(var(--bg-base))' }}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-theme">
          <h3 className="text-sm font-bold text-1">Create Firewall Domain List</h3>
          <button onClick={onClose} className="btn-ghost !p-1.5"><X size={14} /></button>
        </div>

        {/* body */}
        <div className="p-5 space-y-3">
          <p className="text-xs text-4">Create an empty domain list. You can add domains after creation.</p>
          <div>
