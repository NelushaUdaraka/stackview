...
import { Search, Plus, GitBranch, Loader2 } from 'lucide-react'
import type { SwfDomain } from '../../types'

interface Props {
  domains: SwfDomain[]
  selectedDomain: SwfDomain | null
  onSelectDomain: (domain: SwfDomain) => void
...
export default function SwfSidebar({
  domains,
  selectedDomain,
  onSelectDomain,
  onRegisterDomain,
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <span className="text-[10px] font-bold text-4 uppercase tracking-wider">
            Domains {!loading && domains.length > 0 && `(${domains.length})`}
          </span>
        ...
        <div className="relative">
          <Search
            size={12}
            className="absolute left-2.5 top-1/2 -translate-y-1/2 text-3 pointer-events-none"
          />
          <input
            type="text"
            value={search}
          ...
          <div className="flex flex-col items-center justify-center py-10 px-3 text-center">
            {loading ? (
              ...
              <>
                ...
                <p className="text-[10px] text-4 mt-1">Register a domain to get started</p>
              </>
            ) : (
              <>
          ...
          filtered.map((domain) => {
            ...
            return (
              ...
              >
                <GitBranch
                  size={13}
                  className={`mt-0.5 shrink-0 ${isSelected ? 'text-green-500' : 'text-4'}`}
                />
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-xs font-medium truncate leading-snug mb-0.5 ${
                      isSelected ? 'text-1' : isDeprecated ? 'text-4 line-through' : 'text-2'
                    }`}
                  >
                    {domain.name}
                  </p>
                  <div className="flex items-center gap-1.5 justify-between">
                    <p className="text-[9px] text-4 truncate">
                      {domain.workflowExecutionRetentionPeriodInDays
                        ? `${domain.workflowExecutionRetentionPeriodInDays}d retention`
                        : '—'}
                    </p>
      ...
      >
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-1.5 text-xs font-semibold
            bg-green-600 hover:bg-green-500 text-white rounded-lg transition-colors shadow-sm"
        >
          <Plus size={13} />
          Register Domain
        </button>
